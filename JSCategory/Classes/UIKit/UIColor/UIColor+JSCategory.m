...
void JS_RGB2HSL(CGFloat r, CGFloat g, CGFloat b,
...
void JS_HSL2RGB(CGFloat h, CGFloat s, CGFloat l,
...
void JS_RGB2HSB(CGFloat r, CGFloat g, CGFloat b,
...
void JS_HSB2RGB(CGFloat h, CGFloat s, CGFloat v,
                CGFloat *r, CGFloat *g, CGFloat *b) {
    CLAMP_COLOR_VALUE(h);
    CLAMP_COLOR_VALUE(s);
    CLAMP_COLOR_VALUE(v);
...
void JS_RGB2CMYK(CGFloat r, CGFloat g, CGFloat b,
...
void JS_CMYK2RGB(CGFloat c, CGFloat m, CGFloat y, CGFloat k,
                 CGFloat *r, CGFloat *g, CGFloat *b) {
    CLAMP_COLOR_VALUE(c);
    CLAMP_COLOR_VALUE(m);
    CLAMP_COLOR_VALUE(y);
...
void JS_HSB2HSL(CGFloat h, CGFloat s, CGFloat b,
...
void JS_HSL2HSB(CGFloat h, CGFloat s, CGFloat l,
...
+ (UIColor *)js_colorWithHue:(CGFloat)hue
    ...
    JS_HSL2RGB(hue, saturation, lightness, &r, &g, &b);
...
+ (UIColor *)js_colorWithCyan:(CGFloat)cyan
    ...
    JS_CMYK2RGB(cyan, magenta, yellow, black, &r, &g, &b);
...
+ (instancetype)js_colorWithHexString:(NSString *)hexString {
    CGFloat r, g, b, a;
    
    if (js_hexStrToRGBA(hexString, &r, &g, &b, &a)) {
        return [UIColor colorWithRed:r
                               green:g
                                blue:b
                               alpha:a];
...
static inline NSUInteger js_hexStrToInt(NSString *str) {
    uint32_t result = 0;
...
static BOOL js_hexStrToRGBA(NSString *str,
    ...
    //RGB,RGBA,RRGGBB,RRGGBBAA
    if (length < 5) {
        *r = js_hexStrToInt([str substringWithRange:NSMakeRange(0, 1)]) / 255.0f;
        
        *g = js_hexStrToInt([str substringWithRange:NSMakeRange(1, 1)]) / 255.0f;
        
        *b = js_hexStrToInt([str substringWithRange:NSMakeRange(2, 1)]) / 255.0f;
        
        if (length == 4)  *a = js_hexStrToInt([str substringWithRange:NSMakeRange(3, 1)]) / 255.0f;
        
        else *a = 1;
    }
    else {
        *r = js_hexStrToInt([str substringWithRange:NSMakeRange(0, 2)]) / 255.0f;
        
        *g = js_hexStrToInt([str substringWithRange:NSMakeRange(2, 2)]) / 255.0f;
        
        *b = js_hexStrToInt([str substringWithRange:NSMakeRange(4, 2)]) / 255.0f;
        
        if (length == 8) *a = js_hexStrToInt([str substringWithRange:NSMakeRange(6, 2)]) / 255.0f;
...
- (CGColorSpaceModel)js_colorSpaceModel {
    return CGColorSpaceGetModel(CGColorGetColorSpace(self.CGColor));
}

- (NSString *)js_colorSpaceString {
